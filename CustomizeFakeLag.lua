local fakelag_enabled = ui.new_checkbox("AA", "Fake lag", "Enabled BkT Fakelag")
...
local BkT_State = "BkT-Default"
local BkT_weapon_visible = false
...
client.set_event_callback("paint",BkT_visible_enemy)
...
local function status_lagshift(e)
	...
	if multi_select(ui.get(fakelag_slider_trigger), "Avoid Wall Peek") and can_fire_wall == true and BkT_weapon_visible == false then
	...
	if multi_select(ui.get(fakelag_slider_trigger), "Avoid Visible Peek") and BkT_weapon_visible == true then
...
client.set_event_callback("setup_command", function(cmd)
	...
	elseif flags_on_land == true and flags_weapon_switch <= 0 and not in_freeze_time and fired_shoot == false and not flags_climbing_adder then
		BkT_State = "BkT-On Land"
	elseif flags_climbing_adder and not in_freeze_time then
		BkT_State = "BkT-Climbing Ldder"
	elseif in_freeze_time then
		BkT_State = "BkT-Freeze Time"
	end

	local trigger_always_packet = true
	local trigger_send_packet = true
	if multi_select(ui.get(fakelag_send_packet_trigger), "On Standing") and BkT_State == "BkT-Default" then
		trigger_send_packet = trigger_choked >= ui.get(limit)
		trigger_always_packet = false
	end

	if multi_select(ui.get(fakelag_send_packet_trigger), "While Moving") and BkT_State == "BkT-Running" or multi_select(ui.get(fakelag_send_packet_trigger), "While Moving") and BkT_State == "BkT-Slow Motion" then
		trigger_send_packet = trigger_choked >= ui.get(limit)
		trigger_always_packet = false
	end

	if multi_select(ui.get(fakelag_send_packet_trigger), "On Duck") and BkT_State == "BkT-Crouching" then
		trigger_send_packet = trigger_choked >= ui.get(limit)
		trigger_always_packet = false
	end

	if multi_select(ui.get(fakelag_send_packet_trigger), "On High Speed") and BkT_State == "BkT-High Speed" then
		trigger_send_packet = trigger_choked >= ui.get(limit)
		trigger_always_packet = false
	end

	if multi_select(ui.get(fakelag_send_packet_trigger), "On Jump") and BkT_State == "BkT-Air" then
		trigger_send_packet = trigger_choked >= ui.get(limit)
		trigger_always_packet = false
	end

	if multi_select(ui.get(fakelag_send_packet_trigger), "On Exploit") and BkT_State == "BkT-Exploit" then
		trigger_send_packet = trigger_choked >= ui.get(limit)
		trigger_always_packet = false
	end

	if multi_select(ui.get(fakelag_send_packet_trigger), "Weapon Fired") and BkT_State == "BkT-Fired" then
		trigger_send_packet = trigger_choked >= ui.get(limit)
		trigger_always_packet = false
	end

	if multi_select(ui.get(fakelag_send_packet_trigger), "On Use") and BkT_State == "BkT-In Use" then
		trigger_send_packet = trigger_choked >= ui.get(limit)
		trigger_always_packet = false
	end

	if multi_select(ui.get(fakelag_send_packet_trigger), "On Freezetime") and BkT_State == "BkT-Freeze Time" then
		trigger_send_packet = trigger_choked >= ui.get(limit)
		trigger_always_packet = false
	end

	if multi_select(ui.get(fakelag_send_packet_trigger), "Weapon Switch") and BkT_State == "BkT-Weapon Switch" then
		trigger_send_packet = trigger_choked >= ui.get(limit)
		trigger_always_packet = false
	end

	if multi_select(ui.get(fakelag_send_packet_trigger), "Weapon Reload") and BkT_State == "BkT-Weapon Reload" then
		trigger_send_packet = trigger_choked >= ui.get(limit)
		trigger_always_packet = false
	end

	if multi_select(ui.get(fakelag_send_packet_trigger), "On Land") and BkT_State == "BkT-On Land" then
		trigger_send_packet = trigger_choked >= ui.get(limit)
		trigger_always_packet = false
	end

	if multi_select(ui.get(fakelag_send_packet_trigger), "While Climbing Ladder") and BkT_State == "BkT-Climbing Ldder" then
		trigger_send_packet = trigger_choked >= ui.get(limit)
		trigger_always_packet = false
	end
