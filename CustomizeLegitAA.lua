...
local enabled_aa = ui.new_checkbox("AA", "Anti-aimbot angles", "Enabled BkT Custom Legit Anti-Aim")
...
local enabled_legit_fakelag = ui.new_checkbox("AA", "Fake lag", "Enabled BkT Legit Fakelag")
local fakelag_typs = ui.new_combobox("AA", "Fake lag", "Legit Fakelag","BkT Condiction","On Peek Lag")
...
local alter_fakelag_typs = ui.new_combobox("AA", "Fake lag", "BkT Fakelag","Adaptive","Factor","Fluctuate","Maximum")
...
local function syr_visible_enemy(ctx)
	...
	if enemy_entindex == nil or not ui.get(enabled_legit_fakelag) or ui.get(fakelag_typs) == "BkT Condiction" then
		return
	end
...
local function legitaa_subject(c)
	...
	ui.set_visible(alter_normal_lag, ui.get(enabled_legit_fakelag) and ui.get(fakelag_typs) == "BkT Condiction" and visible_adaptive)
	ui.set_visible(alter_send_lag, ui.get(enabled_legit_fakelag) and ui.get(fakelag_typs) == "BkT Condiction" and ui.get(alter_fakelag_typs) == "Fluctuate")
	ui.set_visible(alter_maximum_lag, ui.get(enabled_legit_fakelag) and ui.get(fakelag_typs) == "BkT Condiction" and ui.get(alter_fakelag_typs) == "Factor")
	...
	if ui.get(enabled_legit_fakelag) then
		...
		ui.set_visible(peek_fakelag_limit,ui.get(fakelag_typs) == "On Peek Lag")
			...
			if ui.get(alter_fakelag_typs) == "Adaptive" then
				BkT_limit = adaptive_lag
				isible_adaptive = false
			elseif ui.get(alter_fakelag_typs) == "Factor" then
				BkT_limit = ui.get(fakelag_resert) and ui.get(alter_normal_lag) or ui.get(alter_maximum_lag)
			elseif ui.get(alter_fakelag_typs) == "Fluctuate" then
				BkT_limit = ui.get(fakelag_resert) and ui.get(alter_normal_lag) or ui.get(alter_send_lag) - 1 >= 1 and ui.get(alter_send_lag) - 1 or ui.get(alter_normal_lag)
			elseif ui.get(alter_fakelag_typs) == "Maximum" then
				BkT_limit = ui.get(alter_normal_lag)
			end

		if ui.get(fakelag_typs) == "BkT Condiction" then
			ui.set(fakelag_value_limit, ui.get(duck_peek) and 6 or BkT_limit)
		else
...
client.set_event_callback("setup_command", function(cmd)
	...
	cmd.allow_send_packet = ui.get(fakelag_typs) == "BkT Condiction" and trigger_always_packet or trigger_send_packet
end)
